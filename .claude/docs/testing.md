...
- `test/test_session_start_launcher.py` — compares the Windows `cmd.exe` fallback (only `Git\cmd` and `System32` on PATH) with a direct Bash launch outside a repo, at its root, and in a nested directory containing spaces. Also checks the Claude Bash command on both platforms with an inherited `GIT_PREFIX`. These test command execution, not the host's plugin loader; use `claude --plugin-dir <plugin> --debug-file <log> -p ...` to verify the actual Claude integration.
