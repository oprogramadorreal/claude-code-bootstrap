...
- **`hooks/session-start` is the only host-aware code.** It detects Codex by `PLUGIN_ROOT` being set and equal to `CLAUDE_PLUGIN_ROOT` (Codex sets both, Claude Code only the latter), then switches its own skill mentions to the `$optimus:` form and appends one line carrying the plugin root plus the slash-skill and AskUserQuestion mappings. Skills stay host-neutral by construction: the absolute-path dispatch rule is what lets the same text drive Codex's `spawn_agent` as well as the Agent tool.
...
- **The hook command in `hooks/hooks.json` is bash-first with a git fallback.** `bash "…"` is the whole story on Claude Code, which runs hooks through Git Bash on Windows and a POSIX shell elsewhere. Codex runs Windows hooks through `cmd.exe`, where common Git for Windows installs expose `Git\cmd` without `bash.exe` on `PATH` (or with WSL's first), so `|| git -c "alias.…=!exec bash …" …` launches the script through Git's own shell only when the direct launch cannot. Three constraints shape that line: Claude Code's schema rejects unknown keys, so Codex's `commandWindows` is not used; the alias body must carry no POSIX path lists or `$` expansions, because Git Bash's MSYS runtime rewrites path lists before `git.exe` sees them; and the alias must stay a fallback, because git runs `!` aliases from the repository top level rather than the cwd the hook script inspects.
