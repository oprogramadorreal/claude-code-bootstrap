...
Read-only analysis and the report-don't-pre-filter rule come from the **Agent Constraints** in `references/shared-agent-constraints.md` at the plugin root, which the main skill prepends at dispatch — that section is their only statement.
